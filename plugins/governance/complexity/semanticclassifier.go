...
func (c *SemanticClassifier) resetForCurrentConfigLocked() {
	...
	if c.config == nil || c.config.Semantic == nil {
		...
		if previous != nil {
			c.cleanupEmbeddedNamespaceLocked(previous.store, previous.namespace, false)
		}
		return
	}
...
func (c *SemanticClassifier) runWarmupWorker() {
	...
	for {
		...
		if c.revision != revision {
			c.cleanupEmbeddedNamespaceLocked(store, namespace, true)
			c.mu.Unlock()
			continue
		}
		c.warmCancel = nil
		...
		if err != nil {
			...
			c.logWarmupError(err)
			c.cleanupEmbeddedNamespaceLocked(store, namespace, false)
		} else {
			previous := c.active
			...
			if previous != nil {
				c.cleanupEmbeddedNamespaceLocked(previous.store, previous.namespace, true)
			}
		}
		c.mu.Unlock()
		return
...
func (c *SemanticClassifier) releaseGeneration(generation *semanticGeneration) {
	...
	c.cleanupEmbeddedNamespaceLocked(generation.store, generation.namespace, true)
}
...
//
// keepDesired protects the namespace implied by the latest configuration while
// a stale worker exits. A failed current warmup passes false so its partial,
// unmarked namespace is discarded before a later retry.
func (c *SemanticClassifier) cleanupEmbeddedNamespaceLocked(store vectorstore.VectorStore, namespace string, keepDesired bool) {
	...
	if keepDesired && c.desiredEmbeddedNamespaceLocked() == namespace {
		return
	}
...
// desiredEmbeddedNamespaceLocked returns the private namespace the latest
// configuration is preparing. It prevents a cancelled worker from deleting a
// generation that a newer revision selected again.
func (c *SemanticClassifier) desiredEmbeddedNamespaceLocked() string {
	if c.config == nil || c.config.Semantic == nil || !c.isOwnedStoreLocked(c.store) {
		return ""
	}
	exemplars := semanticExemplars(c.config)
	if len(exemplars) == 0 {
		return ""
	}
	return semanticGenerationNamespace(semanticFingerprint(c.config, exemplars))
}
